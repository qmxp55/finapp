...
def resumen_deuda_tc(gsheet_connector, month=None, year=None):
    ...
    tab_deuda_tc = deuda_tc(
        df=get_data(gsheet_connector, "resumen_pagos_income"),
        month=month,
        year=year,
    )
    tab_ppngi = pago_pngi(
        df=get_data(gsheet_connector, "records"), month=month, year=year
    )

    tab_ppngi = tab_ppngi.loc[:, "Gastos del periodo anterior (GPA)"].drop(
        tipo_de_pago_ed() + ["Total"]
    )
    # tab_ppngi['Total'] = sum(tab_ppngi)
    # st.dataframe(tab_ppngi)

    tab_deuda_tc = tab_deuda_tc.set_index("Tarjeta")
    tab_deuda_tc["Deuda mes siguiente"] = (
        tab_deuda_tc["PPNGI"] - tab_deuda_tc["Pago efectuado"]
    )
...
registros_gastos, registros_ingresos_pagosTC, dashboard = st.tabs(
    ["Registro de Gastos", "Registro de Ingresos y pagos de TC", "Dashboard"]
)
...
with registros_ingresos_pagosTC:
    ...
    with form:
        if entry == "Pago a TC":
            tipo_pago = st.selectbox(
                "Tipo de pago:", tipo_de_pago_tc(), index=0
            )
            ...
            tipo_pago = st.selectbox(
                "Tipo de pago:", tipo_de_pago_ed(), index=0
            )
    ...
    # st.dataframe(get_data(gsheet_connector))
    AgGrid(
        get_data(gsheet_connector, "resumen_pagos_income"),
...
with dashboard:
    ...
    mes = cols[0].selectbox(
        "Mes:", list(meses().keys()), index=int(current_month) - 1
    )
    mes_anterior = cols[0].selectbox(
        "Mes anterior:", list(meses().keys()), index=int(current_month) - 2
    )
    ...
    tab_deuda_tc_mes_anterior = resumen_deuda_tc(
        gsheet_connector, month=meses()[mes_anterior], year=year
    ).round(2)

    st.subheader(f"Resumen pagos a realizar en {mes}")

    tab = deuda_mes(
        df=get_data(gsheet_connector, "records"),
        income=income,
        month=meses()[mes],
        year=year,
    )
    with st.container():
        ...
        gpa.metric(
            "GPA/PPNGI",
            f'{tab.loc["Total GPA", "Monto"]}',
        )
        gma_no_fijos.metric(
            "GMA No Fijos",
            f'{tab.loc["Total GMA No Fijos del periodo", "Monto"]}',
        )
        gmc_fijos.metric(
            "GMC Fijos D/E",
            f'{tab.loc["Total GMA Fijos del periodo en Debito/efectivo", "Monto"]}',
        )
        deuda_tc_mes_anterior.metric(
            "Deuda PPNGI mes anterior",
            f'{tab_deuda_tc_mes_anterior.loc["Total", "Deuda mes siguiente"]}',
        )

        st.dataframe(
            resumen_deuda_tc(
                gsheet_connector, month=meses()[mes], year=year
            ).style.format("{:.2f}")
        )
        # st.dataframe()
        ...
        td.metric(
            "Total Deuda",
            f'{tab.loc["Total necesario para este mes", "Monto"]}',
        )
        ti.metric(
            "Total E/D Ingresos",
            f"{income}",
        ...
        tdisp.metric(
            ...
            f'{tab.loc["Disponible para este mes", "Monto"]}',
        )
    ...
    df_records = get_data(gsheet_connector, "records")

    df_sum, df_fijo, df_msi = expenses_summary(
        df=df_records, month=meses()[mes], year=year
    )
    df_no_fijo = summary_gasto_no_fijo(
        df=df_records, month=meses()[mes], year=year
    )

    gastos_categoria_current = gastos_category(
        df=df_records,
        month=str(int(meses()[mes]) + 1),
        year=year,
    )
    tab_p = pago_pngi(
        df=df_records,
    ...
    st.metric(
        "Total disponible al dia para Gastos No Fijos",
        f"{round(disponible_al_dia, 2)} MXN",
    )
    ...
    cols = st.columns([1, 3])
    gastos_category = gastos_categoria_current.loc["Total", :].drop(
        ["Total", "Gasto fijo"]
    )
    ...
    st.caption(
        "Incluye gastos fijos del Periodo Anterior para pagos con tarjetas de Credito."
    )
    st.caption(
        "Y gastos fijos del mes actual para pagos en Efectivo y/o Debito."
    )
    st.dataframe(
        df_fijo[
            ["establecimiento", "Cargo", "fecha de operacion", "tipo de pago"]
        ]
    )

    st.markdown("#### Gastos a MSI")
    st.caption(
        "Muestra los gastos fijos que estan a Meses Sin Intereses a pagar o que ya se pagaron este mes."
    )
    st.caption(
        "Incluye gastos fijos del Periodo Anterior para pagos con tarjetas de Credito unicamente."
    )
    st.dataframe(
        df_msi[
            ["establecimiento", "Cargo", "fecha de operacion", "tipo de pago"]
    ...
    st.subheader("NOTAS")
    st.markdown(
        "Muestra el total de deuda y el disponible despues de considerar los gastos por tarjeta de credito del periodo anterior y del mes anterior ademas de los gastos fijos en efectivo del mes actual. "
    )
    st.markdown(
        "**GPA/PPNGI**: Total de Gastos del Periodo Anterior que corresponde al Pago Para No Generar Intereses de todas las tarjetas de credito."
    )
    st.markdown(
        "**GMA No Fijos**: Total Gastos del Mes Anterior No Fijos. Corresponde a todos los gastos **No Fijos** fuera del GPA pero que se tienen que cubrir con el efectivo/debito disponible de este mes."
    )
    st.markdown(
        "**GMC Fijos D/E**: Total Gastos del Mes Corriente **Fijos** que se pagan en Efectivo Y Debito."
    )
    st.markdown(
        "**Total Deuda**: Total necesario para cubrir GPA/PPNGI + GMA No Fijos + GMC Fijos D/E."
    )
    st.markdown(
        "**Total E/D Ingresos**: Total Ingresos en Efectivo/Debito de este mes."
    )
    st.markdown(
        "**Total E/D disponible**: Total disponible en Efectivo/Debito de este mes despues de cubrir **Total Deuda**."
    )
